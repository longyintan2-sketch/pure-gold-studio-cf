...
export async function onRequestPost({ request, env }: any) {
  ...
  const row = await env.DB.prepare('SELECT stars, logs FROM keys WHERE key=?').bind(userKey).first();
  ...
  const after = await env.DB.prepare('SELECT stars FROM keys WHERE key=?').bind(userKey).first();
  return json({ ok: true, balance: after.stars, cost });
}
